    ...
    public func addLeftGestures() {
        ...
        if (leftViewController != nil) {
            if leftPanGesture == nil {
              ...
              //  leftPanGesture = UIPanGestureRecognizer(target(forAction: #selector(self.handleLeftPanGesture(_:)), withSender: nil))
            }
...
//        return view.bounds.widthCGRectGetWidth(view.bounds)
    ...
    private func setOpenWindowLevel() {
        if (SlideMenuOptions.hideStatusBar) {
            ...
            DispatchQueue.main.async(execute: {if let window = UIApplication.shared.keyWindow {
    ...
    public func gestureRecognizer(_ shouldReceivegestureRecognizer: UIGestureRecognizer, shouldReceive touch: UITouch) -> Bool {
...
//        }
    ...
    private func isLeftPointContainedWithinBezelRect(point: CGPoint) -> Bool{
        if let bezelWidth = SlideMenuOptions.leftBezelWidth {
            let leftBezelRect: CGRect = .zero
            var _: CGRect = .zero
        
//            CGRectDivide(view.bounds, &leftBezelRect, &tempRect, bezelWidth, CGRectEdge.minXEdge)
            
            let rect = CGRect(x: 0, y: 0, width: bezelWidth, height: 50)
            let (_, _) = rect.divided(atDistance: 5.0, from: .minXEdge)
    ...
    private func slideRightViewForGestureRecognizer(gesture: UIGestureRecognizer, withTouchPoint point: CGPoint) -> Bool {
        return isRightOpen() || SlideMenuOptions.rightPanFromBezel && isRightPointContainedWithinBezelRect(point: point)
    }
    ...
    private func isRightPointContainedWithinBezelRect(point: CGPoint) -> Bool {
        if let rightBezelWidth = SlideMenuOptions.rightBezelWidth {
            let rightBezelRect: CGRect = .zero
            var _: CGRect = .zero
